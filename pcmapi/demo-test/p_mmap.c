...
static unsigned long FREE_SIZE = 200 * 1024 * 1024;
...
int p_init(int size) {
    ...
    /* Initiate the free_size metadata */
    FREE_SIZE = SHM_SIZE;
...
int p_recover() {
    ...
    /* Recovery Procedure
     * 1. Check the metadata of log space to certify all the active transactions T
     * 2. For all t in T: excute step 3 to step 7
     *  3. check t.START and t.END to certify if t is complete
     *  4. If complete, move to next transaction log
     *  5. If incomplete, check t.CNT_PTR and t.FREE_PTR for further undo operations
     *   6. undo update and insert operations with t.CNT_PTR update
     *   7. undo memory malloc operations with t,CNT_FREE update
     * 8. end recovery procedure with a global mark
     */
    // success in recovery
    return 0;
}
...
int p_recover_single(int pid) {
    int iRet;
    // find the log address for corresponding pid
    unsigned long *log_start_addr = (unsigned long *)(pBaseAddr + SHM_SIZE);
    unsigned long *log_use_addr = log_start_addr;
    unsigned long leap = 1024;
    while (pid != *(log_use_addr+1)) {
    ...
    unsigned long *record_addr = log_use_addr + 6;
    unsigned long cnt_mlc = *(log_use_addr + 4);
    unsigned long *last_record_addr = log_use_addr + 2 * cnt_mlc;

    // begin undo operations
    while (cnt_mlc > 0) {
        *(unsigned long *)(*last_record_addr+pBaseAddr) = *(last_record_addr + 1);
        cnt_mlc--;
        last_record_addr -= 2;
    }

    // clear this log for others
    memset(log_use_addr, 0, sizeof(unsigned long)*1024);
